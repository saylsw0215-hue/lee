...
namespace HeroDefense.Save
{
    ...
    public sealed class SaveGameManager:MonoBehaviour
    {
        ...
        public void LoadOrCreate(){var result=Repository.Load();Data=result.Success?result.Data:DefaultSaveFactory.Create();LastLoadRecovered=result.Recovered;if(!new SaveMigrationService().TryMigrate(Data)||!SaveValidationService.Normalize(Data,out _))Data=DefaultSaveFactory.Create();dirty=false;saveDelay=0;ApplySettings();if(!result.Success)SaveNow(SaveReason.Boot);DataChanged?.Invoke();}
