...
namespace HeroDefense.Battle.Effects
{
    ...
    public sealed class FloatingDamageTextPool : MonoBehaviour
    {
        ...
        private Text Acquire(Transform parent){int maximum=SaveGameManager.Instance?.Data.settings.graphicsQuality==GraphicsQualityOption.Low?24:48;if(available.Count>0){active++;return available.Dequeue();}if(created>=maximum)return null;created++;active++;return Create(parent);}
        ...
        private IEnumerator Animate(Text label)
        {
            ...
            label.gameObject.SetActive(false);active=Mathf.Max(0,active-1);available.Enqueue(label);
        }
    }
}
