...
namespace HeroDefense.Battle.Projectiles
{
    ...
    public sealed class ProjectilePool:MonoBehaviour
    {
        ...
        public void Show(UnitVisualShape shape,Transform parent,Vector3 start,Vector3 end)
        {
            ...
            Image image=available.Count>0?available.Dequeue():Create();ActiveCount++;active.Add(image);image.transform.SetParent(parent,false);image.transform.localPosition=start;image.gameObject.SetActive(true);
        ...
        {float elapsed=0f;while(elapsed<.18f){elapsed+=Time.deltaTime;image.transform.localPosition=Vector3.Lerp(start,end,elapsed/.18f);yield return null;}Return(image);}
        public void ReturnAll(){StopAllCoroutines();for(int i=active.Count-1;i>=0;i--)Return(active[i]);}
        private void Return(Image image){if(image==null||!active.Remove(image))return;image.gameObject.SetActive(false);ActiveCount=Mathf.Max(0,ActiveCount-1);available.Enqueue(image);}
    }
}
