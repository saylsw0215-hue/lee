...
namespace HeroDefense.UI.Heroes
{
    ...
    {
        private readonly Dictionary<HeroData,Image> cards=new();private HeroData selected;private Text selectionText;
        private void Start()
        {
            ...
            var start=UiFactory.Button(safe,"StartBattle","다음: 스테이지",new Color(.12f,.48f,.32f));Place(start,new Vector2(.75f,.05f),new Vector2(.95f,.16f));start.onClick.AddListener(()=>{HeroSelectionService.Instance.Select(selected);SceneLoader.Instance.Load(SceneNames.StageSelect);});
            Select(HeroSelectionService.Instance.GetSelectedOrDefault());
        }
        private void AddCard(Transform row,HeroData data)
        {
            var button=UiFactory.Button(row,data.HeroId,"",data.PlaceholderColor);button.GetComponent<LayoutElement>().preferredWidth=280;Image image=button.GetComponent<Image>();cards[data]=image;var unlocks=new HeroUnlockService(SaveGameManager.Instance);button.onClick.AddListener(()=>{if(unlocks.IsUnlocked(data.HeroId))Select(data);else if(unlocks.TryUnlock(data.HeroId))Select(data);else selectionText.text="잠김: "+unlocks.Requirement(data.HeroId);});
            ...
            if(!unlocks.IsUnlocked(data.HeroId)){var locked=UiFactory.Label(button.transform,"Locked","🔒 잠김\n"+unlocks.Requirement(data.HeroId),18,TextAnchor.MiddleCenter,new Color(1,.85f,.55f));locked.raycastTarget=false;locked.rectTransform.anchorMin=new Vector2(.02f,.3f);locked.rectTransform.anchorMax=new Vector2(.98f,.7f);locked.rectTransform.offsetMin=locked.rectTransform.offsetMax=Vector2.zero;}
        }
        private void Select(HeroData data){if(data==null||SaveGameManager.Instance!=null&&!new HeroUnlockService(SaveGameManager.Instance).IsUnlocked(data.HeroId))return;selected=data;CollectionService.Record(data.HeroId,CollectionEvent.Used);foreach(var pair in cards){bool hasPoster=pair.Value.sprite!=null;pair.Value.color=hasPoster?(pair.Key==selected?new Color(1f,.84f,.48f):Color.white):(pair.Key==selected?Color.Lerp(pair.Key.PlaceholderColor,Color.white,.38f):pair.Key.PlaceholderColor);}if(selectionText!=null)selectionText.text=$"선택: {data.DisplayName}";HeroSelectionService.Instance.Select(data);}
