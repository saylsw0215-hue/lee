...
namespace HeroDefense.Tests
{
    public sealed class Phase6AdvancedCombatPlayModeTests
    {
        ...
        [UnityTest]public IEnumerator Silence_DisablesSkillButton(){Select("SeraFireMage");yield return Load();var hero=Hero();Spawn("EnemySlime",hero.transform.localPosition+Vector3.right*100);hero.ApplyStatus(Resources.Load<StatusEffectData>("StatusEffects/Silence"));yield return null;Assert.IsFalse(GameObject.Find("HeroSkill").GetComponent<Button>().interactable);Assert.IsFalse(hero.UseActiveSkill());}
        [UnityTest]public IEnumerator Pause_CancelsAimingAndFreezesStatus(){Select("RianRanger");yield return Load();var hero=Hero();Spawn("EnemySlime",hero.transform.localPosition+Vector3.right*100);hero.ApplyStatus(Resources.Load<StatusEffectData>("StatusEffects/Slow"));float remaining=hero.Statuses.Active[0].Remaining;var aim=Object.FindAnyObjectByType<SkillAimingController>();aim.Begin(hero.Data.ActiveSkill,false);GameObject.Find("Pause").GetComponent<Button>().onClick.Invoke();yield return null;hero.Simulate(2);Assert.IsFalse(aim.IsAiming);Assert.AreEqual(remaining,hero.Statuses.Active[0].Remaining,.01f);}
        ...
        [UnityTest]public IEnumerator CombatReset_RemovesAdvancedState(){Select("ArdenKnight");yield return Load();var hero=Hero();hero.ApplyStatus(Resources.Load<StatusEffectData>("StatusEffects/Slow"));hero.Shields.Add("test",50,5);Combat().ResetBattle();Assert.Zero(hero.Statuses.Active.Count);Assert.Zero(hero.Shields.Total);}
        ...
        private static CombatUnit Spawn(string resource,Vector3 position){var combat=Combat();UnitData data=Resources.Load<UnitData>("UnitData/"+resource);combat.Spawn(data);var units=Object.FindObjectsByType<CombatUnit>(FindObjectsInactive.Exclude);for(int i=units.Length-1;i>=0;i--)if(units[i].Data==data){units[i].transform.localPosition=position;return units[i];}Assert.Fail("Spawned unit not found: "+resource);return null;}
        private static void Select(string resource){if(HeroSelectionService.Instance==null)new GameObject("HeroSelectionService",typeof(HeroSelectionService));HeroSelectionService.Instance.Select(Resources.Load<HeroData>("HeroData/"+resource));}
