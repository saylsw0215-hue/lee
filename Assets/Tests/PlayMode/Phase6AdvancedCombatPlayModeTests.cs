...
namespace HeroDefense.Tests
{
    public sealed class Phase6AdvancedCombatPlayModeTests
    {
        ...
        [UnityTest]public IEnumerator CombatReset_ReturnsActiveHeroEffects(){Select("ArdenKnight");yield return Load();var effects=Object.FindAnyObjectByType<HeroEffectPool>();effects.Show(Vector3.zero,40,Color.white,10);Assert.AreEqual(1,effects.ActiveCount);Combat().ResetBattle();Assert.Zero(effects.ActiveCount);}
        [UnityTest]public IEnumerator AccessibilitySettings_DisableDamageNumbersAndScreenShake(){Select("ArdenKnight");yield return Load();var save=SaveGameManager.Instance;bool oldNumbers=save.Data.settings.damageNumbers,oldShake=save.Data.settings.screenShake;var labels=Object.FindAnyObjectByType<FloatingDamageTextPool>();var targetObject=new GameObject("ShakeTarget",typeof(RectTransform));var target=targetObject.GetComponent<RectTransform>();var shake=targetObject.AddComponent<ScreenShakeController>();shake.Initialize(target);try{save.Data.settings.damageNumbers=false;labels.ShowText(labels.transform,Vector3.zero,"TEST",Color.white);Assert.Zero(labels.ActiveCount);save.Data.settings.screenShake=false;shake.Play(.2f,20);yield return null;Assert.AreEqual(Vector2.zero,target.anchoredPosition);}finally{save.Data.settings.damageNumbers=oldNumbers;save.Data.settings.screenShake=oldShake;Object.Destroy(targetObject);}}
