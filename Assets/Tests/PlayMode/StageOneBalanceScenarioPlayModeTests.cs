...
namespace HeroDefense.Tests.PlayMode
{
    ...
    public sealed class StageOneBalanceScenarioPlayModeTests
    {
        ...
        private static IEnumerator Run(string heroId)
        {
            ...
            using var telemetry=new StageBalanceTelemetrySession(heroId,waves,combat,session);Place("building_barracks","BuildSlot_1",telemetry,100);Place("building_archery_range","BuildSlot_2",telemetry,140);
            bool mageBuilt=false,upgraded=false,ultimateUsed=false;float deadline=Time.realtimeSinceStartup+180f;Time.timeScale=12f;
            while(!combat.IsStageEnded&&Time.realtimeSinceStartup<deadline)
            {
                telemetry.SampleActiveObjects();
                if(!mageBuilt&&session.CurrentGold>=180){Place("building_magic_tower","BuildSlot_3",telemetry,180);mageBuilt=true;}
                if(mageBuilt&&!upgraded&&session.CurrentGold>=100&&waves.Runtime.WaveIndex>=0){int before=session.CurrentGold;Click("BuildSlot_1");Click("Upgrade");if(session.CurrentGold<before){telemetry.RecordGoldSpent(before-session.CurrentGold);upgraded=true;}}
            ...
            Assert.That(combat.IsStageEnded,Is.True,"Stage did not terminate.");Assert.That(telemetry.Report.HasInvalidNumbers(),Is.False);Assert.That(telemetry.Report.bossSpawned,Is.True,"Boss did not spawn.");Assert.That(mageBuilt,Is.True,"Baseline strategy never afforded a magic tower.");Assert.That(waves.Statistics.InstalledBuildings,Is.GreaterThanOrEqualTo(3));Assert.That(waves.Statistics.HeroSkillUseCount,Is.GreaterThan(0));Assert.That(waves.Statistics.HeroUltimateUseCount,Is.GreaterThan(0));Assert.That(upgraded,Is.True,"Baseline strategy never afforded an upgrade.");
        }
