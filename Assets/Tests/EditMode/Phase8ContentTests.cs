...
namespace HeroDefense.Tests.EditMode
{
    public sealed class Phase8ContentTests
    {
        ...
        [Test]public void StageOneBalanceIsLocalAndDifficultyStillHasMultipleAxes(){var first=GameContentDatabase.Stage("stage_01_grassland");var second=GameContentDatabase.Stage("stage_02_red_canyon");Assert.That(first.EnemyHealthMultiplier,Is.GreaterThan(second.EnemyHealthMultiplier));Assert.That(second.EnemyHealthMultiplier,Is.EqualTo(1));Assert.That(DifficultyModifiers.For(GameDifficulty.Easy).EnemyDamage,Is.LessThan(DifficultyModifiers.For(GameDifficulty.Normal).EnemyDamage));Assert.That(DifficultyModifiers.For(GameDifficulty.Hard).EnemySpeed,Is.GreaterThan(DifficultyModifiers.For(GameDifficulty.Normal).EnemySpeed));}
