...
namespace HeroDefense.Tests
{
    public sealed class Phase6AdvancedCombatTests
    {
        ...
        [Test]public void HealingService_AppliesAndPublishesExactlyOnce(){target.Health.TakeDamage(Info(30,DamageType.True));int events=0;HealingInfo received=default;HealingResult published=default;void OnApplied(HealingInfo info,HealingResult result){events++;received=info;published=result;}HealingService.Applied+=OnApplied;try{var result=HealingService.Apply(new HealingInfo(go,target,20,"test_heal"));Assert.IsTrue(result.Applied);Assert.AreEqual(90,target.Health.CurrentHealth,.01f);Assert.AreEqual(1,events);Assert.AreEqual("test_heal",received.SkillId);Assert.AreEqual(20,published.Restored,.01f);}finally{HealingService.Applied-=OnApplied;}}
