...
namespace HeroDefense.Tests
{
    public sealed class Phase4Tests
    {
        ...
        [SetUp]public void Setup(){stage=RuntimeStageCatalog.GetStageOne();first=stage.Waves[0];}
        ...
        [Test]public void Wave_TotalEnemyCountIsAccurate(){Assert.AreEqual(7,first.TotalEnemyCount);Assert.AreEqual(16,stage.Waves[4].TotalEnemyCount);}
        [Test]public void SpawnGroup_RejectsInvalidCountAndInterval(){var bad=new WaveSpawnGroup(RuntimeUnitCatalog.Get("EnemySlime"),0,0,-1,0);Assert.IsFalse(bad.Validate(2,out _));}
        ...
        [Test]public void Wave_DoesNotCompleteWhileEnemyAlive(){var runtime=Start();for(int i=0;i<first.TotalEnemyCount;i++)runtime.RegisterSpawn();Assert.IsFalse(runtime.CanComplete);}
        [Test]public void Wave_CompletesAfterAllEnemiesDie(){var runtime=Start();for(int i=0;i<first.TotalEnemyCount;i++)runtime.RegisterSpawn();for(int i=0;i<first.TotalEnemyCount;i++)runtime.RegisterDeath();Assert.IsTrue(runtime.CanComplete);}
        ...
        [Test]public void EliteHealth_IsHigherThanNormal(){Assert.Greater(RuntimeUnitCatalog.Get("EnemyEliteSlime").MaxHealth,RuntimeUnitCatalog.Get("EnemySlime").MaxHealth);Assert.Greater(RuntimeUnitCatalog.Get("EnemyEliteGoblin").MaxHealth,RuntimeUnitCatalog.Get("EnemyGoblin").MaxHealth);}
        [Test]public void BossDataAndFinalWave_AreValid(){Assert.IsTrue(stage.Waves[9].IsBossWave);Assert.IsTrue(stage.Validate(out _));Assert.AreEqual(2600,RuntimeUnitCatalog.Get("BossGoblinChieftain").MaxHealth);}
        ...
        private WaveRuntimeState Resolved(){var r=Start();for(int i=0;i<first.TotalEnemyCount;i++)r.RegisterSpawn();for(int i=0;i<first.TotalEnemyCount;i++)r.RegisterDeath();return r;}
    }
}
