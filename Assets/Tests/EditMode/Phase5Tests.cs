...
namespace HeroDefense.Tests
{
    public sealed class Phase5Tests
    {
        ...
        [SetUp]public void Setup(){knight=Resources.Load<HeroData>("HeroData/ArdenKnight");}
        [Test]public void HeroData_RequiredValuesValidate(){Assert.IsTrue(knight.Validate(out _));Assert.AreEqual("hero_arden_knight",knight.HeroId);}
        [Test]public void SelectionService_StoresSelectedHero(){var go=new GameObject("Selection",typeof(HeroSelectionService));var ranger=Resources.Load<HeroData>("HeroData/RianRanger");go.GetComponent<HeroSelectionService>().Select(ranger);Assert.AreSame(ranger,go.GetComponent<HeroSelectionService>().SelectedHero);Object.DestroyImmediate(go);}
        ...
        [Test]public void PassiveData_UsesExpectedEventKind(){Assert.AreEqual(HeroPassiveKind.SteelWill,knight.Passive.Kind);Assert.AreEqual(HeroPassiveKind.ConsecutiveShot,Resources.Load<HeroData>("HeroData/RianRanger").Passive.Kind);}
